        ...
        >
          ...
          <v-list-item-action class="pa-0 pl-2 ma-0" v-if="mode !== 'select'">
            <div>
              ...
              <v-btn icon @click="onStarClick($event, clips[index])" @click.right="onStarRightClick($event, clips[index])">
                <v-icon
                  :color="
                    `${
...
export default class Home extends ExtendedVue {
  ...
  public async onStarRightClick(event: Event, clip: Clip): Promise<void> {
    event.stopPropagation();
    console.log(clip)
    await this.modifyClip({
      clip: {
